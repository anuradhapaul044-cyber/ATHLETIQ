  ...
  const filtered = useMemo(
    () =>
      opportunities.filter((opportunity) => {
        ...
        const matchesSearch = !query || opportunity.title.toLowerCase().includes(query) || opportunity.sport.toLowerCase().includes(query) || opportunity.organization.toLowerCase().includes(query);
        return matchesCategory && matchesSearch;
      }),
    [category, search, opportunities],
  );
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-[var(--color-text-muted)] mt-0.5">Trials, scholarships, and programmes relevant to athlete scouting.</p>
      </div>
        ...
        {categories.map((item) => (
          <button
            ...
            className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-all ${
              category === item ? 'bg-[var(--color-brand)] text-white border-[var(--color-brand)]' : 'bg-white border-[var(--color-border)] text-[var(--color-text-secondary)] hover:border-[var(--color-brand)]/50'
            }`}
          >
            {item}
          </button>
          ...
          {selected ? (
            <Card className="sticky top-6">
              <div className="flex items-center gap-2 mb-3">
                <Badge variant={selected.source === 'demo' ? 'warning' : 'success'}>{selected.source === 'demo' ? 'Demo Content' : 'Official Source'}</Badge>
